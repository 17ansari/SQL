/*
Given is the dataset of football players with their basic information.
*/
...
-- Q1 Let’s select the columns FirstName, LastName, and Height from the players table.

select	FirstName,LastName,Height
from players;

-- Q2  Let’s see the Unique Team_Position from the players table.
select distinct(Team_Position)
 from players;
 
 -- Q3 Write a query that filters only those players that have a Height greater than 180cm.
 
 select	firstName,LastName, height
 from players
 where Height >=180;
 
 --  Q4  Let’s see the insights we can get from the club FC Barcelona.

-- Query 1: Let's see how many Barcelona players are in our table.
select count(playerid) as Total_player_in_Baracelona 
from players
where club = 'FC Barcelona';

-- Query 2: Calculate the average height of Barcelona players.
select avg(Height) as AvgHeight_player_in_Baracelona 
from players
where club = 'FC Barcelona';

-- Query 3: Sum the wage of Barcelona players 
select sum(Wage_eur) as TotalWages_player_in_Baracelona 
from players
where club = 'FC Barcelona';

-- Q5  Let’s find out who are the tallest players on our table
select	firstName,LastName,height
from players
order by height desc;

-- Q6 Query: Show players from France (FRA) with a height greater than 180cm.
select	firstName,LastName, height,Nationality
 from players
 where Nationality='FRA' and Height >=180;
 
-- Q7 Query: Show players that are from France (FRA) or that have a height greater than 180cm.
select	firstName,LastName, height,Nationality
 from players
 where  Height >=180;
 
 -- Q8 let’s find out which players are between the ages of 20 and 30.

 select	firstName,LastName, Age
 from players
 where  age >=20 and age<=30;
 -- Q9  Let’s get a list of players who play either for FC Barcelona or Real Madrid.
select firstname,lastname,age,club
from players
where club in('FC Barcelona', 'Real Madrid');
 
 
 -- Q10  Let’s filter those players with names that start with the letter “s”.
